...
*********************
Version Notes:
- Changed keyboard input handling, using termios library to make it
  non-blocking
- Deleted telnet test code block. This can be found in versions 1.7 and earlier
*/
...
{   
   ...
   int select_result = select(STDIN_FILENO + 1, &input_fds, NULL, NULL, &timeout);

   if (select_result > 0 && FD_ISSET(STDIN_FILENO, &input_fds)) 
   {
      char key;
