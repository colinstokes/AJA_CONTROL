...
Reverted to version 1.7 to continue working on issue of loop
breaking after one key input
*/
...
/*// Define flag to control recording thread
int recording_flag = 0;

// Define flag to control stopping thread
int stopping_flag = 0;

// Function to start recording in a separate thread
void *start_recording_thread(void *arg)
{
   // Set the flag to indicate that recording is in progress
   recording_flag = 1;
   start_recording();

   // Reset the flag to indicate that recording has finished
   recording_flag = 0;
   return NULL;
}
*/
